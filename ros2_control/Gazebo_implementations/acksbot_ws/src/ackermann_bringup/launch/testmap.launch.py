...
def generate_launch_description():
    map_path = os.path.join(get_package_share_path('ackermann_bringup'),'maps', 'cafe.yaml')
    rviz_config_path = os.path.join(get_package_share_path('ackermann_bringup'),'rviz_config', 'genmap.rviz')

    
    acks_launch = IncludeLaunchDescription(PythonLaunchDescriptionSource(
        os.path.join(get_package_share_directory('ackermann_bringup'),
                     'launch/ackscont.launch.py')))
